...
export const ClientList = component$(() => {
  const clientLogos = [
    { name: "ASSIFEP", logo: logoFunRadio },
    { name: "FUN Radio", logo: logoFunRadio },
    { name: "Direct Formation", logo: logoFunRadio },
    { name: "M6 Télévision", logo: logoFranceTravail },
      ...
      <div class="flex w-full max-w-7xl flex-col-reverse items-center justify-between gap-10 rounded-3xl bg-[#EEF0FC] px-4 py-8 sm:px-6 md:px-8 lg:flex-row lg:gap-16">
        <div class="w-full text-center lg:w-1/2 lg:text-left">
          ...
          <p class="mb-8 text-sm leading-relaxed text-[#2E104E] sm:text-base md:text-lg">
            Ces entreprises nous accompagnent pour simplifier la mise en
          ...
          <div class="relative h-[400px] w-full overflow-hidden rounded-xl bg-[#EEF0FC] px-2">
            <div class="absolute inset-0 flex flex-col justify-between py-2">
              {Array.from({ length: 5 }).map((_, i) => (
                <div class="clientList-carousel-row h-[64px] overflow-hidden" key={i}>
                  <div
